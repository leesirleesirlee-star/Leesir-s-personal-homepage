...
| **R4 部署** ✅ 2026-09-17 | GitHub 仓库 + Cloudflare Pages（主站）+ GitHub Pages（镜像）+ .nojekyll + 双站全链路回归 | R3 完成 |
...
- **R4-进行中（2026-09-17）**：GitHub 仓库创建（`leesirleesirlee-star/Leesir-s-personal-homepage`）并完成首推（SSH）。推送前安全处置：① git 历史重写，从历史彻底移除 2 个个人 rtf（PRD/个人信息陈述）+ `.opencode/` 工具目录（26 commits 完整、v1.0/v2.0 tags 跟随重写；PRD 由 textutil 全文备份重建，个人陈述原件待本人从其他渠道恢复）；② 11 项敏感信息终审全绿（无私密凭据/手机号/身份证/本地路径/EXIF；anon key 与邮箱属设计公开）；③ `.gitignore` 加固（`.opencode/` 整目录 + 两个 rtf 永不入库）。进展：**双站均已上线并完成 HTTP/内容层回归** ✅：
  - 主站 `https://leesir-s-personal-homepage.pages.dev/`（Cloudflare Pages，自动部署关联 main）
  - 镜像 `https://leesirleesirlee-star.github.io/Leesir-s-personal-homepage/`（GitHub Pages，`.nojekyll` 已就位）
  - 回归结果：两站 5 项核心资源（index/css/js/vendor/portrait）全部 200；头像线上=本地 135,349B；部署版本确认为 v2.4.0（R2.5 字段与 JS 逻辑在线）
...
| V3-Design-04 | 2026-09-17 | §9/§10 回填：R2.5（关系+设备字段）闭环；**R4 双站上线**（Cloudflare 主站 + GitHub Pages 镜像）回归通过 |
